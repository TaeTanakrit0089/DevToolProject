...
def update_events():
    ...
    events = Events.objects.filter(routine__range=(1, 4)).filter(
        Q(noti_date__lt=now_time.date()) | Q(noti_date__lte=now_time.date(), noti_time__lt=now_time.time()))
    for event in events:
    ...
    def get(self, request):
        ...
        if request.user.is_authenticated:
            ...
            today_events = Events.objects.filter(
                Q(user=request.user) | Q(family__in=request.user.family_set.all())
            ).filter(
                noti_date=today
            )
            if user.birth_date:
...
class CalendarView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        return render(request, "calendar.html",
                      {'form': form, 'base_url': base_url, 'google_tts_api_key': settings.GOOGLE_TTS_API_KEY})
...
class FamilyView(LoginRequiredMixin, View):
    ...
    def post(self, request, name):
        try:
            ...
            return JsonResponse({'message': f'Family "{family.name}" created successfully.', 'name': family.name},
                                status=201)
...
class FamilyMemberView(LoginRequiredMixin, View):
    ...
    def delete(self, request, family_id, user_id):
        ...
        if request.user in family.users.all():
            ...
            return JsonResponse({'message': f'Member "{user.username}" removed from family "{family.name}".'},
                                status=200)
